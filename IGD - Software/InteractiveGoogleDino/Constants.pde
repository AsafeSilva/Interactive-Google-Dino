// Global constants

public final int COLOR_BACKGROUND = 30;

public final int CAMERA_WIDTH = 640;
public final int CAMERA_HEIGHT = 480;

public final int WINDOW_WIDTH = 660;
public final int WINDOW_HEIGHT = 500;

public final String INTERNAL_CAM = "HP Webcam-101";
public final String EXTERNAL_CAM = "USB2.0 Camera";

public final int MARGIN = 10;

public final int H = 0;
public final int S = 1;
public final int B = 2;

public final int OBJECT_WIDTH_MIN = 10;
public final int OBJECT_HEIGHT_MIN = 10;

public final int FRAME_INTERVAL = 1;

public final int MIN_DISTANCE = 5;